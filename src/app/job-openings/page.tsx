...
function JobOpeningsPageContent() {
  ...
  const updateModifiedContacts = async (contactFormEntries: ContactFormEntry[]) => {
  ...
  const handleUpdateJobOpening = async (formValues: EditJobOpeningFormValues, openingId: string) => {
    ...
    const contactInputs = formValues.contacts.map(c => ({
      id: c.contact_id || null, // ID can be null for new contacts
      name: c.contactName,
      email: c.contactEmail || null,
      linkedin_url: c.linkedin_url || null,
      phone: c.phone || null,
  ...
  const handleConfirmDeleteOpening = async () => {
    ...
    try {
      ...
      toast({ title: "Lead Deleted", description: `${openingToDelete.role_title} removed.`});
