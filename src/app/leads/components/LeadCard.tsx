...
export function LeadCard({
  ...
  const followUpsArray = useMemo(() =>
    Array.isArray(lead.followUps) ? lead.followUps.map(fu => ({...fu, follow_up_date: new Date(fu.follow_up_date)})) : [],
    [lead.followUps]
  );

  const displayContactNames = useMemo(() => {
    if (!lead.associated_contacts || lead.associated_contacts.length === 0) {
      return 'No contacts linked.';
    }
    return lead.associated_contacts.map(c => c.name).join(', ');
  }, [lead.associated_contacts]);

  const primaryContactDetail = useMemo(() => {
    const firstContact = lead.associated_contacts?.[0];
  ...
  const formattedInitialEmailDate = lead.initial_email_date && isValid(lead.initial_email_date)
    ? format(new Date(lead.initial_email_date), 'PPP') // Ensure date is treated as Date object
    : 'Invalid Date';
  ...
  const handleLogFollowUpClick = async (event: React.MouseEvent) => {
    ...
    try { await onLogFollowUp(nextFollowUp.id, lead.id); }
    catch (error) { }
  ...
  const handleUnlogFollowUpClick = async (event: React.MouseEvent) => {
    ...
    try { await onUnlogFollowUp(lastSentFollowUpLoggedToday.id, lead.id); }
  ...
  const handleToggleFavoriteClick = async (event: React.MouseEvent) => {
    ...
    try {
      await onToggleFavorite(lead.id, !!lead.is_favorite);
    } catch (error) {
    ...
    >
      <CardHeader>
        <div className="flex justify-between items-start">
          <CardTitle className="font-headline text-xl mb-1">{lead.role_title}</CardTitle>
             ...
             <Button
              ...
              >
                {isTogglingFavorite ? (
                  ...
                  <Star className={cn("h-5 w-5", lead.is_favorite ? "fill-yellow-400 text-yellow-500" : "text-muted-foreground")} />
                )}
              </Button>
            <Badge className={cn("capitalize", getStatusBadgeClass(lead.status))}>
              {lead.status}
            </Badge>
          </div>
        </div>
        <CardDescription className="text-primary">{lead.company_name_cache}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-3 text-sm flex-grow">
        {lead.associated_contacts && lead.associated_contacts.length > 0 ? (
        ...
        {lead.job_description_url && (
            <div className="flex items-center">
                <ExternalLink className="mr-2 h-4 w-4 text-muted-foreground" />
                <a href={lead.job_description_url} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline truncate" onClick={(e) => e.stopPropagation()}> Job Description </a>
            </div>
        )}
        {lead.notes && (<p className="text-xs text-muted-foreground pt-1 italic break-words">Notes: {lead.notes}</p>)}
      </CardContent>
